...
class InputBox(pygame.sprite.Sprite):
    def __init__(self, pos, size):
        ...
        self.input = Input(x=pos[0], y=pos[1], color=BLACK )
    ...
    def update(self):
        if self.selected:
            self.text = self.arial.render(self.input.value, False, BLACK)
            self.box.box.blit(self.text, (self.marginX,self.marginY))
